...
export default function App() {
  ...
  const [activeId, setActiveId] = useState<string>(() => {
    ...
    return getActiveEventId(loaded);
  });
  const [gifts, setGifts] = useState<GiftRecord[]>(() => getStoredGifts());
  ...
  const [currentUserIdState, setCurrentUserIdState] = useState<string>(() => {
    ...
    return getStoredCurrentUser(loadedUsers).id;
  });
  ...
  const [isMapsExplorerOpen, setIsMapsExplorerOpen] = useState(false);
  ...
  // Set venue directly from Google Maps Explorer
  const handleSelectVenueForCurrentEvent = (venueTitle: string, address?: string) => {
    if (!currentEvent) {
      showToast('⚠️ សូមជ្រើសរើស ឬបង្កើតកម្មវិធីជាមុនសិន');
      return;
    }
    const fullLocation = venueTitle + (address ? ` (${address})` : '');
    const updatedEvt: EventItem = {
      ...currentEvent,
      location: fullLocation,
    };
    const updated = events.map((e) => (e.id === currentEvent.id ? updatedEvt : e));
    handleEventsChange(updated);
    saveEventToCloud(updatedEvt).catch((err) =>
      console.error('Cloud save event location error:', err)
    );
    showToast(`📍 បានកំណត់ទីតាំង Google Maps៖ «${venueTitle}»`);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col transition-colors duration-200">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-3.5 sm:px-6 lg:px-8 py-4 sm:py-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5 mt-2">
          ...
          <div className="hidden sm:flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
            {activeMainTab === 'RECORDS' ? (
              <span>បង្ហាញបញ្ជីភ្ញៀវ និងការកត់ត្រា</span>
            ) : (
              <span>បង្ហាញ Bar Chart តាមកម្មវិធី & Pie Chart (PAID vs PENDING)</span>
            )}
          </div>
        </div>

        {/* View 1: Guest Gift List Table with Filters & Search */}
        {activeMainTab === 'RECORDS' && (
          <GiftListTable
            gifts={currentEventGifts}
            filters={filters}
        ...
        {/* View 2: Data Visualization Tab (Recharts Bar & Pie Charts) */}
        {activeMainTab === 'ANALYTICS' && (
      ...
      {/* Google Maps Grounding Venue & Nearby Services Explorer Modal */}
      <VenueMapsExplorerModal
        isOpen={isMapsExplorerOpen}
